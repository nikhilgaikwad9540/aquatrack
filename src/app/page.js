  ...
  const handleSelectCustomer = async (cust) => {
    try {
      const [deliveriesSnap, paymentsSnap] = await Promise.all([
        getDocs(collection(db, "customers", cust.id, "deliveries")),
        getDocs(collection(db, "customers", cust.id, "payments")),
      ]);
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      {loading ? (
        ...
        <div className="bg-white border rounded p-4 shadow-md">
          ...
          <p><strong>Building:</strong> {selectedCustomer.building}</p>
          <p><strong>Room:</strong> {selectedCustomer.room}</p>
          <p><strong>Contact:</strong> {selectedCustomer.contact}</p>
          <p><strong>Total Bottles:</strong> {selectedCustomer.totalBottles}</p>
          <p><strong>Bottle Price:</strong> ₹{selectedCustomer.bottlePrice}</p>
          <p><strong>Total Amount:</strong> ₹{selectedCustomer.totalAmount}</p>
